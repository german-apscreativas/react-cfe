      ...
      <Row className='auth-inner m-0'>
        ...
        <Col className='d-flex align-items-center auth-bg px-2 p-lg-5' lg='4' sm='12'>
          <Col className='px-xl-2 mx-auto' sm='8' md='6' lg='12'>
            ...
            <div className='auth-footer-btn d-flex justify-content-center'>
              <Button.Ripple color='facebook' onClick={ async () => {
                try {
                      ...
                      const facebook = await signinWithFb('LOGIN')
                      if (facebook) dispatch({type: 'LOGIN', data: facebook.user})
              ...
              <Button.Ripple color='google' onClick={async () => {
                 dispatch({type: 'LOADING', loading: true})
                 try {
                   const google = await signinWithGoogle('LOGIN')
                   dispatch({type: 'LOADING', loading: false})
                   if (google) dispatch({type: 'LOGIN', data: google.user})
                   else Swal.fire({icon: 'error', text: 'Error al autentiarse'})
                 } catch (e) {
                   dispatch({type: 'LOADING', loading: false})
                 }
                }}
            ...
            <p className='text-center mt-2'>
              ¿Aún no tienes una cuenta?, <Link to="/signup"> <a>Crear cuenta nueva</a></Link>
            </p>
          </Col>
        </Col>
      </Row>
